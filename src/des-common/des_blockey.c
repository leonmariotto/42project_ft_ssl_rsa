...
char                *blockey_msg_exp(int i, char *buf, t_keyring *keys)
{
    char        r[8];
    char        l[8];
    char        *r_exp;
...
char                *blockey_msg_shift_exp(char *r_exp)
{
    int     column;
    int     row;
    char    *ser;
...
int                 des_blockey_pass(int i, char *buf, t_keyring *keys)
{
    ...
    if (!(r_exp = blockey_msg_exp(i, buf, keys)) ||
        !(shift_r_exp = blockey_msg_shift_exp(r_exp)) ||
        !(r_pbox = blockey_rpbox(shift_r_exp)))
        return (0);
    j = 0;
    while (j < 4)
    {
